...
% 转换图像为双精度类型
inputImage = im2double(handles.img);
...
% 生成频域坐标
[x, y] = meshgrid(-paddedCols/2:(paddedCols/2-1), -paddedRows/2:(paddedRows/2-1));
...
% 构造高通滤波器
butterworthFilter = 1 ./ (1 + (cutoffFreq ./ distanceMatrix).^(2 * filterOrder));
butterworthFilter(distanceMatrix == 0) = 0; % 处理零频点

% 扩展图像尺寸并计算 FFT
paddedImage = padarray(inputImage, [rows, cols], 'post'); % 扩展到2倍大小
fftImage = fftshift(fft2(paddedImage));

% 应用滤波器
filteredFFT = fftImage .* butterworthFilter;

% 逆傅里叶变换回空间域
filteredImage = real(ifft2(ifftshift(filteredFFT)));

% 截取回原始图像大小
filteredImage = filteredImage(1:rows, 1:cols);

% 保存结果到 handles
handles.img = filteredImage;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% 检查图像是否是彩色图像，如果是，则转换为灰度图
if size(handles.img, 3) == 3
    grayImage = rgb2gray(handles.img);
else
    grayImage = handles.img;
end

% 将图像转换为双精度类型
inputImage = im2double(grayImage);
...
% 创建理想低通滤波器
lowPassFilter = double(distanceMatrix <= cutoffFreq);

% 对图像进行傅里叶变换
fftImage = fftshift(fft2(inputImage, paddedRows, paddedCols));

% 应用低通滤波器
filteredFFT = fftImage .* lowPassFilter;
